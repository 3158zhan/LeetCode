...
跟动规不同，动规里，单个子问题只能影响父问题，不足以决定父问题。
    从左往右扫描，当遇到重复字母时，以上一个重复字母的\fn{index+1}，作为新的搜索起始位置，直到最后一个字母，
复杂度是O(n)。如图image/longest-substring-without-repeating-characters.png所示。
...
public:
        ...
        return max((int)s.size() - start, max_len); //别忘记最后一次，例如“abcd”
    }
};
